...
def insert_data():
    ...
    # # 병원 위치 API DB 저장하기
    # sql = "CREATE TABLE IF NOT EXISTS LOC_table (Hpname VARCHAR(200), Hpaddr VARCHAR(300), Hplat FLOAT, Hplon FLOAT, Loc_Id INT NOT NULL, PRIMARY KEY(Loc_Id), FOREIGN KEY(Loc_Id) REFERENCES ER_table(Er_Id));"
    # cur.execute(sql)

    # sql = "INSERT INTO LOC_table (Hpname, Hpaddr, Hplat, Hplon, Loc_Id) VALUES (?, ?, ?, ?, ?);"
...
scheduler.add_job(func=insert_data, trigger='interval', minutes=5)
